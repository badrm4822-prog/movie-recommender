...
# ── Step 3 & 5: Feature Engineering + Content Model ──────────
@st.cache_data
def build_content_model(movies_df):
    ...
    return movies_df, cosine_sim, title_to_idx


# ── Step 6: Collaborative Filtering (SVD) ────────────────────
@st.cache_resource
def train_svd(_ratings_df):
...
# ── Step 8: Evaluation ───────────────────────────────────────
@st.cache_data
def evaluate_model(_testset, _svd_model):
    actual_ratings    = [true_r for (_, _, true_r) in _testset]
    predicted_ratings = [_svd_model.predict(uid, iid).est for (uid, iid, _) in _testset]
...
def get_hybrid_recommendations(user_id, movie_title, movies_df, cosine_sim,
                                title_to_idx, svd, alpha=0.5, top_n=10):
    ...
    cf_norm = scaler.fit_transform(cf_arr).flatten()
...
def main():
    ...
    with st.spinner("جاري تحميل البيانات وتدريب النموذج... لحظة من فضلك ⏳"):
        movies_df, ratings_df          = load_data()
        movies_df, cosine_sim, title_to_idx = build_content_model(movies_df)
        svd, testset                   = train_svd(ratings_df)
    ...
    axes[0].bar(['RMSE', 'MAE'], [rmse, mae],
                color=['tomato', 'sandybrown'], edgecolor='black')
    ...
    if st.button('Get Recommendations'):
        hybrid_recs = get_hybrid_recommendations(
            user_id, movie_title, movies_df, cosine_sim, title_to_idx, svd
        )
